    ...
    <v-container>
        <v-row>
            <v-col col="12">
                <v-form v-if="user.status_eva === 2 || user.status_eva === 3">
                    ...
                    <v-row class="mt-2" v-for="(topic, t) in topics" :key="topic.id_topic">
                        <v-col cols="12">
                            ...
                            <v-table class="table">
                                <tr>
                                    <th class="bg-grey border pa-1" style="width: 10%;">ตัวช้วัด</th>
                                    ...
                                    <th class="bg-grey border pa-1" style="width: 10%;">คะแนนทีได้</th>
                                </tr>
                                <tr v-for="indicate in topic.indicates" :key="indicate.id_indicate">
                                    ...
                                    <td class="border pa-1 text-center" style="width: 10%;">{{
                                        scores[indicate.id_indicate?.a ?? 'รอประธานประเมิน'] }}</td>
                                    <td class="border pa-1 text-center" style="width: 10%;">{{
                                        scores[indicate.id_indicate?.b ?? 'รอกรรมการประเมิน'] }}</td>
                                    <td class="border pa-1 text-center" style="width: 10%;">{{
                                        scores[indicate.id_indicate?.c ?? 'รอเลขาประเมิน'] }}</td>
                                    <td class="border pa-1 text-center" style="width: 10%;">{{
                                        (((scores[indicate.id_indicate]?.a ?? 0) + (scores[indicate.id_indicate]?.b ??
                                        0)+(scores[indicate.id_indicate]?.c ?? 0))/3).toFixed(2) }}</td>
                                </tr>
                            </v-table>
                        </v-col>
                    </v-row>
                    <div class="mt-2 text-center">
                        <v-card color="success" class="pa-2 text-end">คะแนนรวม : {{ ((user.total_commit) / 3).toFixed(2)
                            }} คะแนน</v-card>
                    </div>
                    <div class="mt-4">
                        <v-card class="pa-2">
                ...
                <v-alert v-else-if="user.status_eva === 1" type="warning">ยังไม่ได้ประเมิน</v-alert>
...
onMounted(async () => {
    Promise.all([fecth(), fecthTopics(), fetchCommits(), fecthScore()])
})
</script>
